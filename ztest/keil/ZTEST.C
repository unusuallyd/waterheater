...
unsigned char fbr=1;
unsigned char  name00[]={"飞射白鹿雪连天，"};
unsigned char  name01[]={"笑书神侠倚碧鸳。"};
unsigned char  name02[]={"自古英雄多奇逸，"};
unsigned char  name03[]={"金老挥毫尽使然。"};
unsigned char  name10[]={"李白乘舟将欲行，"};
unsigned char  name11[]={"忽闻岸上踏歌声。"};
unsigned char  name12[]={"桃花潭水深千尺，"};
unsigned char  name13[]={"不及汪伦送我情。"};
unsigned char  name20[]={"这是我的128*64液"};
unsigned char  name21[]={"晶显示测试程序，"};
unsigned char  name22[]={"welcome to commu"};
unsigned char  name23[]={"今天的实时温度："};
unsigned char  name24[]={"接收到串口信息："};
unsigned char  name25[]={"EEPROM读取中……"};
unsigned char  name26[]={"读取的EEPROM为："};
...
unsigned char  zi[]={"欢迎使用本产品，本系统是基于高科技产品。"
...
void WRDataXY(uchar x,uchar y, uchar Data,unsigned char color)
{
...
void GCCDRAMCL(unsigned char f)
{
	uchar i,j;
	wr_com(0x34);
	if(f==1)
...
//				  a2表示结束X地址(0-7)
//				  b2表示结束Y地址(0-127)  
void huitu(unsigned char flg1,unsigned char flg2,uchar a1,uchar a2,uchar b1,uchar b2,unsigned char *puts)
{
     unsigned int x=0;
     unsigned char i,j;
...
void WRData(uchar x,uchar y, uchar LData,uchar HData)
{
	wr_com(0x36);	  //切换到扩展指令集并打开绘图模式
	if(y<32)
	{
